...
import {
  ...
  createMountPoint,
} from "../../src/mounter/reactTree";

const MyComponent = ({ text }: any) => (
  <div data-testid="my-component">{text}</div>
);
...
describe("createReactTreeMounter and createMountPoint", () => {
  ...
  test("mounts a component in a specific mount node", () => {
    ...
    let key: string;
    act(() => {
      key = mounterWithNode.mount(MyComponent as any, {
        text: "Hello, world!",
      });
    });

    const mountedComponent = mountNode.querySelector(
    ...
    const unmountedComponent = mountNode.querySelector(
      '[data-testid="my-component"]'
    );
    expect(unmountedComponent).not.toBeInTheDocument();
  });
